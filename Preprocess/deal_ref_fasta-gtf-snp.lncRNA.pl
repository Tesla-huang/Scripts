...
if(-s "$op.gtf" and -s "${op}_lnc.gtf" and -s "${op}_code.gtf")
{
...
while(<GTF>)
{
	...
	{
		$bgl{$1} = 0 if($tmp[8] =~ /transcript_id "([^;]+)";/);
		push @{$gse{$tmp[0]}{$tmp[3]}{$tmp[4]}}, $_;
		push @{$all{$tmp[0]}{$tmp[3]}{$tmp[4]}}, $_;
	}elsif($tmp[8] =~ /transcript_biotype "antisense";/ or $tmp[8] =~ /transcript_biotype "lincRNA";/ or $tmp[8] =~ /transcript_biotype "lncRNA";/)
	{
		$bgl{$1} = 0 if($tmp[8] =~ /transcript_id "([^;]+)";/);
		push @{$nc{$tmp[0]}{$tmp[3]}{$tmp[4]}}, $_;
		push @{$all{$tmp[0]}{$tmp[3]}{$tmp[4]}}, $_;
	}
}
...
open GTFO, "> ${op}_code.gtf" or die $!;
open NC, "> ${op}_lnc.gtf" or die $!;
open NORMAL, "> $op.gtf" or die $!;
foreach(@chrs)
{
	foreach my $i(sort {$a<=>$b} keys %{$all{$_}})
	{
		foreach my $j(sort {$a<=>$b} keys %{$all{$_}{$i}})
		{
			print NORMAL join("\n", @{$all{$_}{$i}{$j}})."\n";
		}
	}
	...
	foreach my $i(sort {$a<=>$b} keys %{$nc{$_}})
	{
		foreach my $j(sort {$a<=>$b} keys %{$nc{$_}{$i}})
		{
			print NC join("\n", @{$nc{$_}{$i}{$j}})."\n";
		}
	}
...
if(-s "${op}_refGene.txt" and -s "${op}_refGeneMrna.fa")
{
...
#`sed -i 1d $op.gff`;
`/Bio/User/linyifan/script/Preprocess/gtfToGenePred -genePredExt ${op}_code.gtf $op.genepred`;
