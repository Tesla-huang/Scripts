...
#	my $lncRNAseq=substr($hash{$lncRNA},$lncRNAsi[0]-1,$lncRNAsi[1]-$lncRNAsi[0]+1);
	...
	my $lnc_5 = $lncRNAsi[0] - 1;
	my $lnc_3 = length($hash{$lncRNA}) - $lncRNAsi[1];
	my $mRNAseq=substr($hash{$mRNA},$mRNAsite[0]-1-$lnc_3,$lnc_5+$mRNAsite[1]-$mRNAsite[0]+$lnc_3+1);
	my $seq="$lncRNAseq&$mRNAseq";
	$seq=~tr/atcg/ATCG/;
	my $align = shift(@array);
	my @align = split /&/, $align;
	$align[0] = " "x$lnc_5.$align[0]." "x$lnc_3;
	$align[1] = " "x$lnc_3.$align[1]." "x$lnc_5;
	print OUT"$lncRNA\n$mRNA\t@array\n$seq\n$align[0]&$align[1]\n\n";
}
